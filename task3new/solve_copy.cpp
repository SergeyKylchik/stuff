...
void solve (int n, double *array, double *x, int *err, int total_p, int my_rank, int matrix_norm, int colomns, 
            double *cosphi_copy, double *sinphi_copy, int *flag_copy)
{
    using std::abs;
    ...
    double *sinphi = sinphi_copy;  //new (std::nothrow) double[n];
    double *cosphi = cosphi_copy;  //new (std::nothrow) double[n];
    int *flag = flag_copy; //new (std::nothrow) int[n];
    for (int i; i<n; i++)
    {
        flag[i] = 0;
    }
